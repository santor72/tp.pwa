...
Отчёт подключения содержит два независимых текста. Текст ТехПортала и фотографии
необязательны; для ремонта текст ТехПортала обязателен, а фотографии необязательны.
Текст GIS используется только при выбранном объекте карты. Если объект выбран и нет
...
Видимость вкладки карты задаёт `GIS_VISIBLE_TECHPORTAL_ROLES`, а выбор объекта
при выполнении заявки — `GIS_TIKETS_VISIBLE_TECHPORTAL_ROLES`. Это списки
исходных ролей (поле `status`) из ТехПортала через запятую; сравнение выполняется
без учёта регистра. Пользователь вне первого списка не видит экран карты, вне
второго — выбор объекта в заявке и не может передать `feature_id`. Чтение GIS
данных доступно пользователю, которому разрешён хотя бы один из этих сценариев.
