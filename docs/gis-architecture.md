...
Комментарий ТехПортала — `techportal_text` и отдельные строки ссылок на фотографии.
Допустим комментарий только из фотографий: в этом случае он содержит ссылки.
`gis_text` в ТехПортал не передаётся. В GIS передаётся только `gis_text` и исходные
файлы фотографий; `techportal_text` в GIS не передаётся.
