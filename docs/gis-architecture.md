...
Отчёт подключения содержит непустой текст или хотя бы одну фотографию.
Backend отклоняет пустой отчёт до записи в ТехПортал. `feature_id` необязателен
для выполнения заявки и обязателен только для отправки в GIS. Это заменяет
прежнее требование обязательного объекта карты.
...
время работ, текст, ключи/хеши/размеры фото и сформированный комментарий ТехПортала.
...
Комментарий ТехПортала — текст отчёта и отдельные строки ссылок на фотографии.
Допустим отчёт только из фотографий: комментарий в этом случае содержит ссылки.
...
1. Сохранить операцию, данные отчёта, ключи фото и итоговый комментарий в нашей БД.
...
текст/фото → «Объект GIS — необязательно» → кнопка «Отметить выполненной».
