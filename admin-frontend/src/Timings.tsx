...
type Detail = Item & { spans: Span[] }
type Data = { items: Item[]; total: number; stats: { samples: number; median_ms: number | null; p95_ms: number | null; failed: number } }
...
export default function Timings() {
  ...
  async function load(next = 1) {
    ...
    try {
      ...
      setData(await get<Data>(`/api/admin/payment-timings?${query}`)); setPage(next)
  ...
  return <section className="shell">
    ...
    {data && <>
      ...
      <div className="table-wrap"><table><thead><tr><th>Создана</th><th>Сотрудник / телефон</th><th>Статус</th><th>Очередь</th><th>Этапы до ссылки</th><th>До ссылки*</th><th>До QR на телефоне</th><th>REST / повторы</th></tr></thead><tbody>
        {data.items.map(item => <tr key={item.id} onClick={() => open(item.id)}><td>{date(item.created_at)}</td><td>{item.employee || '—'}<small>{item.phone}</small></td><td>{item.status}</td><td>{ms(item.queue_ms)}</td><td>{ms(item.formation_ms)}</td><td>{ms(item.link_ms)}</td><td>{ms(item.qr_ms)}{item.browser_flags.some(f => f.background || f.restored) && <small>Фон / восстановление</small>}</td><td>{item.measured ? `${item.rest_calls} / ${item.rest_retries}` : 'Нет замера'}<small>Повторы worker: {item.worker_retries}</small></td></tr>)}
      </tbody></table></div>
