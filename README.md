...
Используйте оба Compose-файла и при последующих пересборках API и
`completion-worker`: оверлей подключает их к сети GIS и задаёт адрес `http://gis:18770`.
...
Для фотоотчётов заполните `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` и
`S3_BUCKET`. `S3_PHOTO_USAGE=gis_and_techportal` (значение по умолчанию)
добавляет публичные ссылки S3 в комментарий ТехПортала; для этого нужен
`S3_PUBLIC_BASE_URL`. При `S3_PHOTO_USAGE=gis_only` S3 хранит фото только для
отправки в GIS, а публичный URL не требуется. `S3_ENDPOINT_URL` может указывать на
...
Фото подключения и ремонта отправляет слой `TechPortalPhotoDelivery` через все настроенные
адаптеры по порядку. S3-адаптер `ObjectStorage` возвращает публичную ссылку
для комментария. `TP_PHOTO_UPLOAD_ENABLED=true` включает адаптер ТехПортала:
он отправляет файл в `/api/tickets/upload-file` с cookies пользователя,
ТехПортал сам показывает прикреплённый файл, поэтому адаптер не добавляет
текст в комментарий. Если настроенный адаптер не сохранил фотографию, запись комментария
останавливается с ошибкой. Чтобы добавить хранилище, реализуйте интерфейс
`PhotoStorageAdapter` и зарегистрируйте адаптер в `create_application_services`.
Для отчёта с фотографиями и выбранным объектом GIS настроенный S3 обязателен:
повторная отправка в GIS читает сохранённые фотографии именно из S3. В режиме
`gis_only` для комментария ТехПортала нужен текст или включённый фотоадаптер ТехПортала.
Ремонт завершается через тот же отчёт, что и подключение: можно выбрать объект GIS
и приложить фотографии. Для ремонта описание выполненных работ обязательно даже
при наличии фотографий; фотографии остаются необязательными.

`GIS_VISIBLE_TECHPORTAL_ROLES` ограничивает экран карты списком значений роли
ТехПортала через запятую, например `admin,manager,user`.
`GIS_TIKETS_VISIBLE_TECHPORTAL_ROLES` отдельно ограничивает выбор объекта GIS
при выполнении заявок. Пустое значение соответствующей переменной отключает
этот сценарий для всех пользователей.

Для карты задайте `GIS_MAP_PROVIDER=yandex` и `YANDEX_MAPS_API_KEY` в `.env`.
`GIS_MAP_PROVIDER` выбирает реализацию карты: `yandex` использует API 2.1 со
схемой и гибридом, `yandex-v3` — API v3 только с векторной схемой. Ключ передаётся браузеру
только через защищённый endpoint PWA для загрузки официального JavaScript API
Яндекс Карт выбранной версии; не добавляйте его в frontend-переменные. В кабинете Яндекса
разрешите production-домен PWA; для API v3 ограничение по HTTP Referer должно
быть заполнено. CSP reverse proxy должен разрешать ресурсы
`https://api-maps.yandex.ru` и домены, которые использует SDK; service worker
не должен кэшировать эти ресурсы.

При закрытии подключения текстовый отчёт и фотографии необязательны. Для ремонта
текстовый отчёт обязателен, фотографии остаются необязательными. В режиме
`gis_only` загрузка фото появляется при выборе объекта GIS.
Это не влияет на фотографии
...
Для загрузки фотоотчётов у внешнего Nginx в соответствующем `server` или
`location /api/` задайте `client_max_body_size 55m;` и перезагрузите конфигурацию.
Такой же лимит задан в `frontend/nginx.conf`: backend допускает до пяти
фотографий по 10 МиБ, а multipart-запросу нужен запас для текста и служебных полей.
Лимит должен быть не меньше 55 МиБ на каждом прокси перед API. Иначе прокси
может вернуть HTTP 413 до обращения к backend. После изменения конфигурации
в репозитории пересоберите и пересоздайте сервис `frontend`.
