...
`GIS_MAP_PROVIDER` выбирает зарегистрированную реализацию карты; сейчас доступен
`yandex`. Ключ передаётся браузеру
только через защищённый endpoint PWA для загрузки официального JavaScript API
Яндекс Карт 2.1; не добавляйте его в frontend-переменные. В кабинете Яндекса
разрешите production-домен PWA. CSP reverse proxy должен разрешать ресурсы
